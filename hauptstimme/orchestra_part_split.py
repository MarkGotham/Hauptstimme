...
from hauptstimme.constants import REPO_PATH, CORPUS_PATH
...
def split_part(
    ...
    for n in new_part_2.recurse().notesAndRests:
        if n.isChord:
            ...
            for i in range(1, len(pitches)):
                # remove all pitches except the first (lowest) one
                n.remove(pitches[i])
        if n.lyric:  # should all be duplicates in this expansion.
            print(
                f"*** removing lyric {n.lyric} from {new_part_2.partName}, measure {n.measureNumber}")
            n.lyric = None
    ...
    if handle_part_name:
        try:
            ...
            trans = ""
            # Lead with "Bb Clarinet" to help MuseScore's bad instrument recognition algorithm.
            if i.transposition:
                trans = pitch.Pitch("C").transpose(
                    i.transposition).name.replace("-", "b")
...
def split_one(
        path_to_score: Path = f"{REPO_PATH}/test/split_test_score_in.mxl",
    ...
    for p in score.parts:
        ...
        if "BrassInstrument" in i.classes:
            ...
            try:
                ...
                pass
                print(
                    f"Could not parse instrument name {p.partName}, skipping")
            score.insert(0, p)
            ...
            if "Rest" in n.classes:
                ...
                if "Rest" not in n.previous().classes:
                    if n.offset == n.previous().offset:
                        m.remove(n)
                        print(rest_template +
                              f"m.{n.measureNumber} (rest listed after).")
                if n.next() and ("Rest" not in n.next().classes):
                    if n.offset == n.next().offset:
                        m.remove(n)
                        print(rest_template +
                              f"m.{n.measureNumber} (rest listed before).")
        ...
        if do_measure_length:
            ...
            for measure in p.getElementsByClass(stream.Measure):
                ...
                if ts:
                    ...
                    print(
                        f"New time signature duration = {current_ts_duration}")
...
def split_corpus(
        sub_corpus_path: str = CORPUS_PATH,
) -> None:
    """
