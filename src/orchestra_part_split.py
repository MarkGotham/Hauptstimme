...
def transposition_check(part: Part):
    ...
    """
    try:
        instr_name = instrument.fromString(part.partName)
        if instr_name is not None:
            trans_from_name = instr_name.transposition
    except:
        print(
            f"Warning: Could not parse instrument name '{part.partName}', " +
...
def expand_scores(
    ...
    Args:
        corpus_sub_dir: The path to a subdirectory within the corpus to
            get files from. Default = DATA_PATH.
    """
