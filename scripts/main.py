...
Run 'python3 main.y {score_file} {args}', where {args} can include:
- -o {output_dir}: The path to the directory in which to save the 
    score files. The default is the score's directory.
- -r {restrictions}: The annotation restrictions - either a regex
    string or list of acceptable values for annotations.
- -t: A boolean flag for if the Hauptstimme annotations are text
    expressions.
"""
...
from src.annotations import get_annotations_and_melody_score
from src.score_conversion import score_to_lightweight_df
from src.part_relations import get_part_relationship_summary
from src.utils import ms3_convert, get_compressed_measure_map
