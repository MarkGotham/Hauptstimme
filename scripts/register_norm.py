...
def register_to_counter(
    ...
    Args:
        ...
        int_column (str): Name of the data frame column containing pitch midi number
            (integer values to be used as keys for the `Counter` object).
        float_column (str): Name of the data frame column containing duration
            (float values to sum for the values of the `Counter`).
        step_size (int): The step size for x-axis data.
            Default is 1: one per MIDI note.
        ticks_step_size (int): The step size for x-axis ticks.
            Default is 12: one per octave starting at the lowest note.
        norm_by_pc (bool): If True, normalise by pitch class.
        plot_dir (Optional[Path]): If provided (not None) plot write to this directory.
            In all cases, return the Counter object.
    ...
    for x in x_values:
        y_values.append(
            counter.get(x, 0.0)  # x-value, or 0 if x not used (not in the counter)
        )
    ...
    if plot_dir is not None:
        ...
        plt.grid(True)
        plt.xticks(np.arange(int(min_val), int(max_val) + 1, ticks_step_size))
        plt.tight_layout()
        if norm_by_pc:
            file_name = "register_use_norm_by_pc.pdf"
        else:
            file_name = "register_use.pdf"
...
def demo():
    """
    Demonstration case based on the `demo_files` on this repo.
    Retrieves that data and saves plots to the same directory.
    """
    current_dir = Path(__file__).parent
    SAMPLE_FILE = current_dir.parent / "notebooks" / "demo_files" / "Beethoven_Op.36_1_note_events.csv"
    data = pd.read_csv(SAMPLE_FILE)
    register_to_counter(data, norm_by_pc=True, plot_dir=SAMPLE_FILE.parent)
